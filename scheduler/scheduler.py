...
def main():
    print(f"Custom-Scheduler: {get_timestamp()}: Starting custom scheduler...")
    w = watch.Watch()
    for event in w.stream(v1.list_namespaced_pod, "default"):
        if event['object'].status.phase == "Pending" and\
            ...
            try:
                ...
                print(f"Custom-Scheduler: {get_timestamp()}: Scheduling result: {res.status}")
                print(f"Custom-Scheduler: {get_timestamp()}: Scheduled {pod_name} to {random_node}")
            except client.rest.ApiException as e:
                print(f"Custom-Scheduler: {get_timestamp()}: An exception occurred: {json.loads(e.body)['message']}")
